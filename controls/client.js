...
const deleteItem = async (req, res) => {
  try {
    await ClientSchema.deleteOne({ _id: req.params.id });
...
const getItems = async (req, res) => {
  try {
    ...
    const options = {
      page: Number(page),
      limit: Number(limit),
      select: `name email phone isActive createdAt order`,
      sort: { order: 1 },
    };
...
const sortItem = async (req, res) => {
  try {
    ...
    const currentRecord = await ClientSchema.findOne({ _id: req.params.id });
    if (!currentRecord) {
      return res.status(404).json({ message: "Category not found" });
    }

    const aboveRecord = await ClientSchema.findOne({
      order:
